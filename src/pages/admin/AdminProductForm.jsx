...
export default function AdminProductForm() {
  ...
  const inputCls = "w-full bg-[#0f172a] border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {form.images.map((url, i) => (
                      <div key={i} className="relative aspect-square rounded-xl overflow-hidden bg-[#0f172a] group">
                        ...
                        {i === 0 && (
                          <span className="absolute bottom-1 left-1 text-xs bg-indigo-600 text-white px-1.5 py-0.5 rounded font-bold">
                            Main
                          </span>
                        )}
                      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-[#1e293b] p-5 rounded-3xl border border-slate-800 space-y-4">
                ...
                <div>
                  ...
                  <input name="price" type="number" value={form.price} onChange={handle} required min="0" className={`${inputCls} font-bold`} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">
                    Original Price (₹)
                  </label>
                  <input name="originalPrice" type="number" value={form.originalPrice} onChange={handle} min="0" className={inputCls} />
                ...
                <div>
                  ...
                  <input name="stock" type="number" value={form.stock} onChange={handle} required min="0" className={`${inputCls} font-bold`} />
                </div>
              </div>
