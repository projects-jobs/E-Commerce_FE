...
import {
  ...
  Package, AlertCircle, ArrowUpRight, LogOut
} from 'lucide-react'
...
function KartifyLogo({ size = 32 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none">
      <rect x="2" y="2" width="36" height="36" rx="10" fill="#6366f1"/>
      <path d="M8 12h3l2 12h14l2-9H13" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
      <circle cx="18" cy="28" r="2" fill="#fff"/>
      <circle cx="26" cy="28" r="2" fill="#fff"/>
      <path d="M20 8l2 3-2 3-2-3z" fill="#c7d2fe"/>
    </svg>
  )
}
    ...
    <div className="min-h-screen bg-[#0f172a] flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#0b1120] border-r border-slate-800 p-6 shrink-0">
        <div className="flex items-center gap-3 mb-10">
          <KartifyLogo size={36}/>
          <div>
            <p className="text-white font-bold text-sm" style={{ fontFamily:'Syne,sans-serif' }}>Kartify</p>
                ...
                {loading ? (
                  [...Array(4)].map((_,i) => (
                    <tr key={i} className="border-b border-slate-800/50">
                      {[...Array(5)].map((_,j) => <td key={j} className="px-6 py-4"><div className="h-3 bg-slate-700 rounded animate-pulse"/></td>)}
                    </tr>
                  ))
                ) : orders.length === 0 ? (
                  ...
                  orders.map(o => (
                    <tr key={o._id} className="border-b border-slate-800/50 hover:bg-white/2 transition-colors">
