...
export default function CartPage() {
  ...
  if (items.length === 0) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center">
      <div className="w-24 h-24 rounded-3xl bg-brand-50 flex items-center justify-center text-5xl mb-6">🛒</div>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      <button onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-sm text-ink-400 hover:text-ink-900 mb-8 transition-colors">
        <ArrowLeft size={16}/> Continue Shopping
      </button>

      <h1 className="text-3xl font-bold text-ink-900 mb-8">
        Your Cart <span className="text-ink-400 font-normal text-xl">({totalItems} {totalItems === 1 ? 'item' : 'items'})</span>
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map((item, idx) => {
            ...
            if (!p) return null
            return (
              <div key={p._id || idx} className="card p-4 sm:p-5 flex gap-4 sm:gap-5">
                {/* Image */}
                <Link to={`/product/${p._id}`}
                  className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-ink-50 shrink-0">
                  {p.images?.[0]
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between gap-2">
                    <Link to={`/product/${p._id}`}
                      className="font-semibold text-ink-900 hover:text-brand-600 transition-colors line-clamp-2 text-sm sm:text-base">
                      {p.name}
                    </Link>
                    <p className="font-bold text-ink-900 shrink-0 text-sm sm:text-base">
                      ₹{(p.price * qty).toLocaleString()}
                    </p>
                  </div>
                  <p className="text-xs text-ink-400 mt-1">₹{p.price?.toLocaleString()} each</p>

                  <div className="flex items-center justify-between mt-4">
                    {/* Qty */}
                    <div className="flex items-center border border-ink-200 rounded-xl overflow-hidden">
                      <button onClick={() => qty > 1 ? updateItem(p._id, qty - 1) : removeItem(p._id)}
                        className="px-3 py-1.5 hover:bg-ink-50 transition-colors text-ink-600">
                        <Minus size={14}/>
                      </button>
                      <span className="px-3 py-1.5 font-semibold text-sm min-w-[2rem] text-center">{qty}</span>
                      <button onClick={() => updateItem(p._id, qty + 1)}
                        disabled={qty >= p.stock}
                        className="px-3 py-1.5 hover:bg-ink-50 transition-colors text-ink-600 disabled:opacity-40">
                        <Plus size={14}/>
                      </button>
                    </div>
        ...
        <div className="lg:col-span-1">
          <div className="card p-6 sticky top-24">
            <h2 className="text-lg font-bold text-ink-900 mb-5 flex items-center gap-2">
              <ShoppingBag size={18} className="text-brand-500"/> Order Summary
            </h2>
