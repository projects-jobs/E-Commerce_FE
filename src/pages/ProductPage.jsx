...
const ProductPage = () => {
  ...
  // Example data based on your Mongoose Schema
  const product = {
    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="p-6 flex justify-between items-center bg-white sticky top-0 z-50 border-b border-slate-50">
        <button className="flex items-center gap-2 text-slate-500 hover:text-black transition-colors">
          <ArrowLeft size={20} /> <span className="text-sm font-medium font-mono uppercase tracking-widest">Back to Collection</span>
        </button>
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
              {product.images.map((img, idx) => (
                <button 
                  ...
                  className={`w-24 h-24 rounded-2xl overflow-hidden border-2 flex-shrink-0 transition-all ${mainImage === img ? 'border-indigo-600 scale-95' : 'border-transparent opacity-60 hover:opacity-100'}`}
                >
        ...
        <div className="lg:w-1/2 p-6 lg:p-12 lg:border-l border-slate-100">
          <div className="max-w-md">
               ...
               <div className="flex gap-3">
                  {['Standard', 'Pro Max', 'Lite'].map((opt) => (
                    <button 
                      ...
                      className={`px-6 py-3 rounded-2xl font-bold text-sm transition-all border-2 ${selectedSize === opt ? 'bg-slate-900 text-white border-slate-900 shadow-xl shadow-slate-200' : 'bg-white text-slate-600 border-slate-100 hover:border-slate-300'}`}
                    >
                      {opt}
                    </button>
                  ))}
            ...
            <div className={`mb-8 flex items-center gap-2 font-bold text-xs p-3 rounded-2xl w-fit ${product.stock > 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
               <div className={`w-2 h-2 rounded-full animate-pulse ${product.stock > 0 ? 'bg-emerald-500' : 'bg-red-500'}`}></div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button className="w-full bg-slate-900 text-white py-5 rounded-[2rem] font-black text-lg flex items-center justify-center gap-3">
                BUY IT NOW
              </button>
            </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map(rev => (
              <div key={rev} className="bg-slate-50 p-8 rounded-[2rem]">
                 ...
                 <p className="text-slate-700 font-medium mb-4 italic">"The sound quality is honestly better than the industry leaders. The design is just a bonus!"</p>
